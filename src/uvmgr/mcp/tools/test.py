...
async def run_tests(
    ...
    try:
        ...
        return OperationResult(
            ...
            details={"verbose": verbose, "output": result if result else "No test output"},
        ).to_string()

    except Exception as e:
        return OperationResult(
            success=False, message="Failed to run tests", details={"error": str(e)}
        ).to_string()
...
async def run_coverage(ctx: Context) -> str:
    ...
    try:
        ...
        return OperationResult(
            success=False, message="Failed to run coverage", details={"error": str(e)}
        ).to_string()
...
async def lint_code(
    ...
    try:
        ...
        return OperationResult(
            success=False, message="Failed to run linting", details={"error": str(e), "path": path}
        ).to_string()
